...
pub const StorageInjector = struct {
    ...
    pub fn dumpChanges(self: *StorageInjector, evm: anytype) ![]const u8 {
        var buffer = std.array_list.Managed(u8){ .items = &[_]u8{}, .capacity = 0, .allocator = self.allocator };
        defer buffer.deinit();
        ...
        // Storage changes
        var first = true;
        var storage_it = self.dirty_storage.iterator();
        while (storage_it.next()) |entry| {
            ...
            // Get original value (from evm.original_storage) and new value (from evm.storage or cache)
            const original_value = evm.original_storage.get(key) orelse 0;
            const new_value = self.storage_cache.get(key) orelse evm.storage.get(key) orelse 0;

            // Use bufPrint for hex formatting
            var addr_buf: [45]u8 = undefined; // "0x" + 40 chars
            const addr_str = try std.fmt.bufPrint(&addr_buf, "0x{x}", .{key.address.bytes});

            var slot_bytes: [32]u8 = undefined;
            std.mem.writeInt(u256, &slot_bytes, key.slot, .big);
            var slot_buf: [67]u8 = undefined; // "0x" + 64 chars
            const slot_str = try std.fmt.bufPrint(&slot_buf, "0x{x}", .{slot_bytes});

            var orig_bytes: [32]u8 = undefined;
            std.mem.writeInt(u256, &orig_bytes, original_value, .big);
            var orig_buf: [67]u8 = undefined;
            const orig_str = try std.fmt.bufPrint(&orig_buf, "0x{x}", .{orig_bytes});

            var new_bytes: [32]u8 = undefined;
            std.mem.writeInt(u256, &new_bytes, new_value, .big);
            var new_buf: [67]u8 = undefined;
            const new_str = try std.fmt.bufPrint(&new_buf, "0x{x}", .{new_bytes});

            try writer.print("{{\"address\":\"{s}\",\"slot\":\"{s}\",\"originalValue\":\"{s}\",\"newValue\":\"{s}\"}}", .{ addr_str, slot_str, orig_str, new_str });
        }
        ...
        // Balance changes
        first = true;
        var balance_it = self.dirty_balances.iterator();
        while (balance_it.next()) |entry| {
            ...
            const new_balance = self.balance_cache.get(address) orelse evm.balances.get(address) orelse 0;

            var addr_buf: [45]u8 = undefined;
            const addr_str = try std.fmt.bufPrint(&addr_buf, "0x{x}", .{address.bytes});

            var balance_bytes: [32]u8 = undefined;
            std.mem.writeInt(u256, &balance_bytes, new_balance, .big);
            var balance_buf: [67]u8 = undefined;
            const balance_str = try std.fmt.bufPrint(&balance_buf, "0x{x}", .{balance_bytes});

            try writer.print("{{\"address\":\"{s}\",\"originalBalance\":\"0x0\",\"newBalance\":\"{s}\"}}", .{ addr_str, balance_str });
        }
        ...
        // Nonce changes
        first = true;
        var nonce_it = self.dirty_nonces.iterator();
        while (nonce_it.next()) |entry| {
            ...
            const new_nonce = self.nonce_cache.get(address) orelse evm.nonces.get(address) orelse 0;

            var addr_buf: [45]u8 = undefined;
            const addr_str = try std.fmt.bufPrint(&addr_buf, "0x{x}", .{address.bytes});

            try writer.print("{{\"address\":\"{s}\",\"originalNonce\":0,\"newNonce\":{}}}", .{ addr_str, new_nonce });
        }
        ...
        // Code changes
        first = true;
        var code_it = self.dirty_codes.iterator();
        while (code_it.next()) |entry| {
            ...
            const code = self.code_cache.get(address) orelse evm.code.get(address) orelse &[_]u8{};

            var addr_buf: [45]u8 = undefined;
            const addr_str = try std.fmt.bufPrint(&addr_buf, "0x{x}", .{address.bytes});

            const code_buf = try self.allocator.alloc(u8, code.len * 2 + 2);
            defer self.allocator.free(code_buf);
            const code_str = try std.fmt.bufPrint(code_buf, "0x{x}", .{code});

            try writer.print("{{\"address\":\"{s}\",\"code\":\"{s}\"}}", .{ addr_str, code_str });
        }
